...
class Strip(PixelStrip):
    def __init__(self, num, gpio, **kwargs):
        ...
        self.num = num
        self.gpio = gpio
        super().__init__(self.num, self.gpio, **kwargs)
        self.begin()
        ...
        if isinstance(key, slice):
            ...
            for i in key:
                if i in range(0, self.num):
                    self.setPixelColor(i, color.dec)
        elif isinstance(key, int):
            if key in range(0, self.num):
                self.setPixelColor(key, color.dec)
        else:
    ...
    def __getitem__(self, key):
        if isinstance(key, slice):
            key = range(key.start if key.start is not None else 0,
                        key.stop if key.stop is not None else self.num,
                        key.step if key.step is not None else 1)
            return [Color(self.getPixelColor(i)) for i in key]
        elif isinstance(key, int):
            return Color(self.getPixelColor(key))
        else:
